...
def accelerate_evaluate(model_name, reward_fn, args, tokenizer, eval_data):
    ...
    for task in tasks_list:
        dataloader = DataLoader(
            eval_data[task], batch_size=32, shuffle=True, collate_fn=collate_fn
        )
        ...
        if accelerator.is_main_process:
            print(
                f"eval_t0/reward_{task}: {mean_reward},\n eval_t0/std_{task}: {mean_stdev}"
            )
            wandb.log(
                {
                    "task": task,
                    f"eval_t0/reward_{task}": mean_reward,
                    f"eval_t0/std_{task}": mean_stdev,
                }
            )
    if accelerator.is_main_process:
        print(
            f"eval_t0/reward: {np.mean(np.array(data))},\n eval_t0/std: {np.std(np.array(data))}"
        )
        wandb.log(
            {
                "eval_t0/reward": np.mean(np.array(data)),
                "eval_t0/reward_std": np.std(np.array(data)),
            }
        )
    ...
    if False:
        ...
        for task in tasks_list:
            ...
            sampling_params = SamplingParams(
                temperature=float(args.eval_temp),
            ...
            for i, output in enumerate(outputs):
                ...
                scores.append(score)
                generation_len.append(len(output.outputs[0].token_ids))
            data.extend(scores)
            lens_data.extend(generation_len)
            mean_reward = np.mean(np.array(scores))
            mean_stdev = np.std(np.array(scores))
            mean_len = np.mean(np.array(generation_len))
            stdev_len = np.std(np.array(generation_len))
            print(
                f"eval_t0/reward_{task}: {mean_reward},\n eval_t0/std_{task}: {mean_stdev}"
            )
            wandb.log(
                {
                    ...
                    f"eval_t0/reward_{task}": mean_reward,
                    f"eval_t0/std_{task}": mean_stdev,
                    f"eval_t0/mean_len_{task}": mean_len,
                    f"eval_t0/std_len_{task}": stdev_len,
                }
            )
        print(
            f"eval_t0/reward: {np.mean(np.array(data))},\n eval_t0/std: {np.std(np.array(data))}"
        ...
        wandb.log(
            {
                "eval_t0/reward": np.mean(np.array(data)),
                "eval_t0/reward_std": np.std(np.std(data)),
                "eval_t0/len": np.mean(np.array(lens_data)),
                "eval_t0/len_std": np.std(np.std(lens_data)),
            }
        )
        print("Evaluation done!")
